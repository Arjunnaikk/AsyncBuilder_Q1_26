...
impl<'info> Withdraw<'info> {
    pub fn withdraw(
        ...
        require!(self.user_lp.amount >= amount,AmmError::LiquidityLessThanMinimum);
        ...
        {       
            true => (min_x, min_y),
            false => {
                let amounts = ConstantProduct::xy_withdraw_amounts_from_l(   //issue fixed
                    self.vault_x.amount,
                    self.vault_y.amount,
                    self.mint_lp.supply,
                    amount,
                ...
                .map_err(AmmError::from)?;                                              //issue fixed
                (amounts.x, amounts.y)
            }
        };
